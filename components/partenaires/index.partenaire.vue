...
<template>
   ...
   <div class="w-11/12 m-auto mt-24" v-if="useDataStore().partners.length != 0">
      <div class="w-full flex justify-center">
         <span class="text-xl text-center">Ils nous font confiance en tant que partenaires.</span>
      </div>
   </div>



   <Carousel class="relative w-11/12 m-auto  pt-4 pb-24" v-if="useDataStore().partners.length != 0" :plugins="[plugin]"
      @mouseenter="plugin.stop" @mouseleave="[plugin.reset(), plugin.play(), console.log('Runing')]">
      <CarouselContent class="flex w-full">
         <CarouselItem v-for="(_, index) in useDataStore().partners" class="pl-1 basis-1/2 lg:basis-1/4">
            <div class="p-1 h-full flex items-center">

               <NuxtImg :src="_.original_url" class="h-16 " alt="" />
